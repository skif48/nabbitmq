...
export class Consumer implements RabbitMqPeer {
  ...
  private fillEmptyConfigsWithDefaults(rawConfigs?: ConsumerConfigs): ConsumerConfigs {
    ...
    filledConfigs.queue.options = filledConfigs.queue.options || {durable: true};
    if (typeof filledConfigs.queue.options.durable === 'undefined')
      filledConfigs.queue.options.durable = true;
    ...
    if (typeof filledConfigs.noDeadLetterQueue !== 'undefined' && filledConfigs.noDeadLetterQueue === true) {
      ...
      filledConfigs.deadLetterQueue.queue.options = filledConfigs.deadLetterQueue.queue.options || {durable: true};
      ...
      filledConfigs.deadLetterQueue.exchange.options = filledConfigs.deadLetterQueue.exchange.options || {durable: true};
  ...
  private async defaultSetup(connection: Connection): Promise<Channel> {
    ...
    if (this.configs.deadLetterQueue) {
      await channel.assertExchange(this.configs.deadLetterQueue.exchange.name, this.configs.deadLetterQueue.exchange.type, this.configs.deadLetterQueue.exchange.options);
      const dlqMetadata = await channel.assertQueue(this.configs.deadLetterQueue.queue.name, this.configs.deadLetterQueue.queue.options);
      await channel.bindQueue(dlqMetadata.queue, this.configs.deadLetterQueue.exchange.name, this.configs.deadLetterQueue.queue.bindingPattern);
    }
    ...
    const queueMetadata = await channel.assertQueue(this.configs.queue.name, {
      ...this.configs.queue.options,
      deadLetterExchange: this.configs.deadLetterQueue ? this.configs.deadLetterQueue.exchange.name : undefined,
      deadLetterRoutingKey: this.configs.deadLetterQueue ? this.configs.deadLetterQueue.queue.bindingPattern : undefined,
    });

    await channel.bindQueue(queueMetadata.queue, this.configs.exchange.name, this.configs.queue.bindingPattern, this.configs.queue.options.arguments);
