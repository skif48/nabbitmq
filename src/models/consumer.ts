...
export class Consumer<T> implements RabbitMqPeer {
  ...
  public async init(connection: RabbitMqConnection): Promise<void> {
    ...
    if (this.configs.prefetch) this.channel.prefetch(this.configs.prefetch);

    this.channel.consume(
      this.configs.queue.name,
      (message: any): void => {
        if (message === null) return void this.subject.error(new Error('Channel was cancelled by the server'));
        else this.subject.next(message);
      },
    ...
    amqpConnection.on('close', () =>
      this.subject.error(new RabbitMqConnectionClosedError('AMQP server closed connection')),
    );
